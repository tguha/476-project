...
glm::mat4 setTextProj(std::shared_ptr<Program> curShade, int width, int height) {
    glm::mat4 proj = glm::ortho(0.0f, static_cast<float>(width), 0.0f, static_cast<float>(height));
    glUniformMatrix4fv(curShade->getUniform("projection"), 1, GL_FALSE, glm::value_ptr(proj));
    return proj;
}
...
void RenderText(std::shared_ptr<Program> textProg, std::string text, float x, float y, float scale, glm::vec3 color,
    ...
    if (textProg->hasUniform("textColor")) {
    ...
    for (c = text.begin(); c != text.end(); c++) {
        ...
        glBindBuffer(GL_ARRAY_BUFFER, TextVBO);
        glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(vertices), vertices); // sizeof(vertices) is the size of the data in bytes
